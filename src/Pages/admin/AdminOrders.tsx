...
const StatusDropdown = ({
    orderId, current, onChange,
}: {
    orderId: number; current: string; onChange?: (s: string) => void;
}) => {
...
// ─── Order Card ───────────────────────────────────────────────────────────────
const OrderCard = ({ order, onView }: { order: any; onView: () => void }) => {
            ...
            {isLoading ? (
                ...
                <div className="bg-white rounded-2xl border border-gray-100 py-24 text-center">
                    ...
                    {search && (
                        <button
                            onClick={() => setSearch("")}
                            className="mt-3 text-xs font-black text-black underline underline-offset-2"
                        >
                            Clear search
                        </button>
                    )}
