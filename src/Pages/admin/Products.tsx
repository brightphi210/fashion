            ...
            <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
                ...
                <div className="p-6 space-y-5">
                    ...
                    <div className="aspect-square rounded-2xl overflow-hidden bg-gray-50 relative">
                        ...
                        {discount && (
                            <span className="absolute top-3 left-3 text-xs font-black text-white bg-red-500 px-2.5 py-1 rounded-lg">-{discount}%</span>
                        )}
                    </div>
                    {/* Info grid */}
                    <div>
                    ...
                    {sizes.length > 0 && (
                        <div>
                            ...
                            <div className="flex gap-1.5 flex-wrap">
                                {sizes.map(s => <span key={s} className="px-2.5 py-1 text-xs font-black border-2 border-gray-200 rounded-xl text-gray-700">{s}</span>)}
                            </div>
                        </div>
                    )}
                    {colors.length > 0 && (
                        <div>
                            ...
                            <div className="flex gap-1.5 flex-wrap">
                                {colors.map(c => <span key={c} className="px-2.5 py-1 text-xs font-black bg-gray-100 border border-gray-200 rounded-full text-gray-700">{c}</span>)}
                            </div>
                        </div>
                    )}
                </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }}>
        <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <div className="flex gap-3">
                <button onClick={onClose} className="flex-1 py-2.5 border-2 border-gray-200 rounded-xl text-sm font-black text-gray-600 hover:bg-gray-50 transition-colors">Cancel</button>
    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all hover:-translate-y-0.5 group flex flex-col">
            ...
            <div className="aspect-square bg-gray-50 overflow-hidden relative">
                ...
                {discount && (
                    <span className="absolute top-2.5 left-2.5 text-[9px] font-black text-white bg-red-500 px-1.5 py-0.5 rounded-md uppercase">-{discount}%</span>
                )}
                {product.tag && (
        ...
        <div className="p-6 max-w-6xl">
            {(showAdd || editTarget) && (
                <AddEditProductModal
                    ...
                    editProduct={editTarget ?? undefined}
                />
            )}
            {viewTarget && (
                <ProductDetailModal
            ...
            {deleteTarget && (
                <DeleteModal product={deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} loading={deleteProduct.isPending} />
            )}
            ...
            <div className="relative mb-6">
                ...
                {search && (
                    <button onClick={() => setSearch("")} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"><FiX size={14} /></button>
                )}
            </div>
                    ...
                    {[...Array(8)].map((_, i) => (
                        <div key={i} className="bg-white rounded-2xl border border-gray-100 overflow-hidden animate-pulse">
                            ...
                            <div className="p-3.5 space-y-2">
                                ...
                                <div className="flex justify-between"><div className="h-4 w-16 bg-gray-100 rounded-full" /><div className="h-4 w-12 bg-gray-100 rounded-full" /></div>
                            </div>
                        </div>
                    ))}
                </div>
